...
var RESOURCE_PATH = "/rev/resource/js/forms/";
...
var Forms = (function(args){
	...
	*/
	var __construct = (function(){
		var els = $JQueryEl.find("[name]:not([type='submit']), [name]:not([data-type='submit'])");
		els.each(function(i){
	...
	var validate = (function(el, me){
		...
		var but = (el.data("but")) ? (el.data("but").toString()) : (undefined);
		...
		if (but != undefined && but.split(";").indexOf(value) != -1) {
			me.status = false;
			return {
				status: me.status,
				msg: args.lang[1],
		...
		if (equal != undefined && $equal.length != 0 && value != $equal.val()) {
			me.status = false;
			return {
				status: me.status,
				msg: args.lang[3],
		...
		if (pattern != undefined && !el.val().match(pattern) || value == "") {	//	validação de nulo é feita em 'required'
			me.status = false;
			return {
				status: me.status,
				msg: args.lang[8],
		...
		switch(type) {
			...
			case "mediumtel":
			case "smalltel":
				if (el.val().length < 8 && value != "") {	// aqui ele pode ser nulo, se ele não pudesse, não teria passado na validação de require
					me.status = false;
					return {
						status: me.status,
			...
			case "email":
				if (!value.match(/^([0-9a-zA-Z]+([_.-]?[0-9a-zA-Z]+)*@[0-9a-zA-Z]+[0-9,a-z,A-Z,.,-]*[.]{1}[a-zA-Z]{2,4})+$/) || el.val() == "") {
					me.status = false;
					return {
						status: me.status,
