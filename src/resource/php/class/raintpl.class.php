...
class RainTPL{
	...
	private function check_template( $tpl_name = NULL ){
		...
		if( !isset($this->tpl['checked']) ){
			...
			$this->tpl['tpl_dir'] 				= $this->tpl_dir . $this->tpl['tpl_basedir'];								// template directory
			$this->tpl['tpl_filename'] 			= $this->tpl['tpl_dir'] . $this->tpl['tpl_basename'] . '.' . TPL_EXT;			// template filename
			$this->tpl['cache_dir'] 			= $this->cache_dir . $this->tpl['tpl_basedir'];									// cache directory
	...
	private function path_replace( $html, $tpl_basedir ){
		...
		// $sub = array( 'src=@http://$1@', 'src=@$1@', 'src="' . $this->base_url . $this->tpl_dir . $tpl_basedir . '\\1"', 'src="$1"', '<link$1href=@http://$2@', '<link$1href=@$2@' , '<link$1href="' . $this->base_url . $this->tpl_dir . $tpl_basedir . '$2"', '<link$1href="$2"' );

		return $html; //preg_replace( $exp, $sub, $html );
	}
